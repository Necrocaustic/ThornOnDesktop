...
class EasterCog(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if 'baka' in content:
            await message.reply('OwO you silly little baka, that word is a bad word UwU, can\'t have that here!')
            await message.delete()
    @commands.command()
    async def hint(self, ctx):
        await ctx.reply('Your hint for today is:')
        await ctx.send('This was a previously banned word')

    @commands.command()
    async def hint2(self, ctx):
        await ctx.reply('Your second hint of the day is:')
        await ctx.send('okay seriously you shouldn\'t need this lmao')
    @commands.command()
    async def hint3(self, ctx):
        await ctx.reply('Your third **__and final__** hint of the day is:')
        await ctx.send('fine if you really fucking need it for whatever reason: it\'s that cringe ass motherfucking word'
                       ' that starts with b and ends in a')
